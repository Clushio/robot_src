...
class SerialTagReader:
    def __init__(self, port='/dev/ttyUSB0', baudrate=38400, max_buffer_size=2048):
        ...
        # ROS 初始化
        rospy.init_node('serial_tag_reader', anonymous=True)
        self.publisher = rospy.Publisher('/tag_position', PoseStamped, queue_size=10)
    ...
    def send_udp_message(self, message):
        try:
            ...
            rospy.loginfo(f"UDP message sent: {udp_message}")
    ...
    def check_receiver_status(self):
        ...
        try:
            ...
            if data.decode('utf-8') == '{"status":"pong"}':
                rospy.loginfo("Receiver is online.")
                return True
        except socket.timeout:
            rospy.logwarn("Receiver did not respond. It might be offline.")
        except Exception as e:
            rospy.logerr(f"Error checking receiver status: {e}")
        return False
    ...
    def parse_data_frame(self, data):
        ...
        if data[1] == ord(self.TAG_PRESENT):
            ...
            if xl > 0 and yl > 0:
                ...
                rospy.logwarn("Error: XL or YL is zero.")
                with self.position_lock:
            ...
            product_type = data[4]
            rospy.loginfo(f"No Tag - Version: {version}, Status: {status}, Error Type: {error_type}, Product Type: {product_type}")
            with self.position_lock:
                self.latest_position = None
    ...
    def run_serial(self):
        try:
            ...
            rospy.logerr(f"Error opening port: {e}")
        ...
        while not rospy.is_shutdown():
            with self.position_lock:
                if self.latest_position:
                    ...
                    rospy.loginfo(f"Published position: x={pose_msg.pose.position.x:.2f}, y={pose_msg.pose.position.y:.2f}, yaw={math.degrees(yaw):.2f}")
