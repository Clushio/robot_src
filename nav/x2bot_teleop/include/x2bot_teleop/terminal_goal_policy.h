#ifndef X2BOT_TELEOP_TERMINAL_GOAL_POLICY_H
#define X2BOT_TELEOP_TERMINAL_GOAL_POLICY_H
...
public:
    ...
    TerminalGoalUpdate update(uint8_t state, bool have_yaw,
                              double yaw, double now)
    {
        TerminalGoalUpdate result;
        if (state == TERMINAL_POSITION_CAPTURED ||
            state == TERMINAL_ROTATING || state == TERMINAL_COMPLETE)
        {
            locked_ = true;
        }
