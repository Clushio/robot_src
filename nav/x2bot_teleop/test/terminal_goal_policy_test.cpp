...
TEST(TerminalGoalPolicy, LocksOnCapturedStateAndIgnoresTrackingTimeout)
{
    ...
    EXPECT_FALSE(policy.update(x2bot_teleop::TERMINAL_TRACKING,
                               true, 0.0, 100.0).locked);
    EXPECT_TRUE(policy.update(x2bot_teleop::TERMINAL_POSITION_CAPTURED,
                              true, 0.0, 101.0).locked);
    EXPECT_FALSE(policy.update(x2bot_teleop::TERMINAL_TRACKING,
                               true, 0.0, 200.0).error_activated);
...
TEST(TerminalGoalPolicy, ReportsOnceThenRecoversOnMeasuredYawProgress)
{
    ...
    EXPECT_FALSE(policy.update(x2bot_teleop::TERMINAL_ROTATING,
                               true, 0.05, 69.9).error_activated);
    EXPECT_TRUE(policy.update(x2bot_teleop::TERMINAL_ROTATING,
                              true, 0.05, 70.0).error_activated);
    EXPECT_FALSE(policy.update(x2bot_teleop::TERMINAL_ROTATING,
                               true, 0.05, 80.0).error_activated);

    const x2bot_teleop::TerminalGoalUpdate recovered =
        policy.update(x2bot_teleop::TERMINAL_ROTATING,
                      true, 0.11, 81.0);
...
TEST(TerminalGoalPolicy, RearmsTimeoutAfterRecoveryAndHandlesYawWrap)
{
    ...
    EXPECT_FALSE(policy.update(x2bot_teleop::TERMINAL_ROTATING,
                               true, -3.12, 10.0).error_activated);
    EXPECT_TRUE(policy.update(x2bot_teleop::TERMINAL_ROTATING,
                              true, -3.12, 30.0).error_activated);
    EXPECT_TRUE(policy.update(x2bot_teleop::TERMINAL_ROTATING,
                              true, -3.00, 31.0).error_recovered);
    EXPECT_TRUE(policy.update(x2bot_teleop::TERMINAL_ROTATING,
                              true, -3.00, 61.0).error_activated);
}
