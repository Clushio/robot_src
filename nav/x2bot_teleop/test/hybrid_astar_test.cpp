...
TEST(HybridAStar, PlansForwardStraightConnector)
{
  ...
  goal.x = 1.5;
  goal.yaw = 0.0;
  const x2bot_teleop::HybridAStarResult result =
      planner.plan(start, goal, boundedFree);
...
TEST(HybridAStar, RoutesAroundBlockedStraightLine)
{
  ...
  const x2bot_teleop::HybridAStarResult result =
      planner.plan(start, goal, obstacle);
  ASSERT_TRUE(result.success) << result.reason;
  bool left_centerline = false;
  for (const x2bot_teleop::HybridPose &pose : result.poses)
  {
...
TEST(HybridAStar, GuidePointReductionKeepsEndsAndTurns)
{
  ...
  const std::vector<x2bot_teleop::HybridPose> guides =
      x2bot_teleop::HybridAStar::guidePoints(path, 0.4, 0.05);
