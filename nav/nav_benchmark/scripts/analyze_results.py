...
def build_summary(directories):
    ...
    for directory in directories:
        ...
        try:
            with open(os.path.join(directory, 'session.json'), 'r', encoding='utf-8') as handle:
    ...
    cte = numeric(samples, 'cte_abs_m', lambda row: row.get('task_state') == 'running')
    ...
    for row in tasks:
        ...
        if result == 'arrived':
            if finite(row.get('settled_position_error_m')):
                group['position_errors_m'].append(float(row['settled_position_error_m']))
            if finite(row.get('settled_yaw_error_deg')):
                group['yaw_errors_deg'].append(float(row['settled_yaw_error_deg']))
    ...
    for group in by_direction.values():
        ...
        group['position_error_m'] = summarize(group.pop('position_errors_m'))
    ...
    return {
        ...
        'observed_replan_count': int(sum(numeric(tasks, 'observed_replan_count'))),
        'collision_slowdown_count': int(sum(numeric(tasks, 'collision_slowdown_count'))),
        'collision_stop_count': int(sum(numeric(tasks, 'collision_stop_count'))),
        'unexpected_stop_count': int(sum(numeric(tasks, 'unexpected_stop_count'))),
        'sharp_turn_count': int(sum(numeric(tasks, 'sharp_turn_count'))),
        'localization_gap_count': int(sum(numeric(tasks, 'localization_gap_count'))),
...
def write_text(path, summary):
    ...
    def metric(group, key, scale=1.0, suffix=''):
        value = group.get(key)
        return 'N/A' if value is None else '{:.3f}{}'.format(value * scale, suffix)
    ...
    lines = [
        ...
        'Collision slowdown/stops: {}/{}'.format(
            summary['collision_slowdown_count'], summary['collision_stop_count']
        ),
        'Manual-intervention tasks: {}'.format(summary['manual_intervention_tasks']),
    ]
...
def main():
    ...
    output_dir = (
        os.path.abspath(os.path.expanduser(arguments.output_dir))
        if arguments.output_dir else directories[0]
    )
