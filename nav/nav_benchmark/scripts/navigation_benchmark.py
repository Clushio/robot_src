...
class NavigationBenchmark(Node):
    def __init__(self):
        ...
        self.sample_rate = max(1.0, float(self._param('sample_rate', 10.0)))
        self.sync_interval = max(0.1, float(self._param('sync_interval', 1.0)))
        self.settle_window = max(0.0, float(self._param('settle_window', 0.5)))
        self.bspline_status_timeout = max(
            0.1, float(self._param('bspline_status_timeout', 0.5))
        )
        self.tracking_min_speed = max(
            0.0, float(self._param('tracking_min_speed', 0.02))
        )
        self.reference_entry_projection_epsilon = max(
            0.0, float(self._param(
                'reference_entry_projection_epsilon', 1e-6
            ))
        )
        self.stop_speed = max(0.0, float(self._param('stop_speed', 0.02)))
        self.stop_duration = max(0.1, float(self._param('stop_duration', 0.5)))
        self.sharp_omega = max(
            0.0, float(self._param('sharp_turn_omega', 0.3))
        )
        self.sharp_duration = max(
            0.1, float(self._param('sharp_turn_duration', 0.3))
        )
        self.max_pose_jump = max(
            0.05, float(self._param('max_pose_jump', 0.5))
        )
        self.localization_gap_threshold = max(
            0.1, float(self._param('localization_gap_threshold', 0.5))
        )
        self.disk_warning_mb = max(
            10.0, float(self._param('disk_warning_mb', 512.0))
        )
        self.recovery_window = max(
            1.0, float(self._param('recovery_window', 30.0))
        )
        self.map_frame = self._param('map_frame', 'map')
        self.base_frame = self._param('base_frame', 'base_link')
        self.positions_file = os.path.expanduser(self._param(
            'positions_file', '~/maps/robot_positions.txt'
        ))
        self.topology_file = os.path.expanduser(self._param(
            'topology_file', '~/maps/topology.yaml'
        ))
        self.results_root = os.path.expanduser(self._param(
            'results_root', '~/maps/nav_benchmark_results'
        ))
        requested_repo_path = self._param('repo_path', '')
        ...
        self.tf_buffer = tf2_ros.Buffer(
            cache_time=Duration(seconds=10.0), node=self
        )
        self.tf_listener = tf2_ros.TransformListener(
            self.tf_buffer, self, spin_thread=False
        )

        # Keep explicit references without shadowing Node.subscriptions, which
        # is a read-only rclpy introspection property in ROS 2 Humble.
        self._subscriptions = [
            self.create_subscription(
                String, '/anav/task_status', self.task_status_callback, 20
            ),
            self.create_subscription(
                Odometry, '/Odometry', self.localization_callback, 1
            ),
            self.create_subscription(Odometry, '/odom', self.odom_callback, 1),
            self.create_subscription(
                Twist, '/cmd_vel/nav', self.cmd_nav_callback, 1
            ),
            self.create_subscription(
                ArbitratedCommand, '/cmd_vel/candidate',
                self.candidate_callback, 1
            ),
            self.create_subscription(
                Twist, '/cmd_vel', self.cmd_output_callback, 1
            ),
            self.create_subscription(
                UInt8, '/anav/topology_safety_phase',
                self.topology_phase_callback, 1
            ),
            self.create_subscription(
                Bool, '/anav/fixed_route_mode', self.fixed_route_callback, 1
            ),
            self.create_subscription(
                Vector3Stamped, '/bspline_status', self.bspline_callback, 1
            ),
            self.create_subscription(
                Path, '/reference_path', self.reference_path_callback, 1
            ),
            self.create_subscription(
                Path, '/topology_plan', self.topology_path_callback, 1
            ),
            self.create_subscription(
                DiagnosticArray, '/diagnostics', self.diagnostics_callback, 20
            ),
        ]

        self.sample_timer = self.create_timer(
            1.0 / self.sample_rate, self.sample_timer_callback
        )
        self.heartbeat_timer = self.create_timer(
            1.0, self.heartbeat_timer_callback
        )
        self.record_event(
        ...
        self.get_logger().info(
            f'Navigation benchmark logging to {self.session_dir}'
        )
    ...
    def _discover_repo_path(candidates):
        ...
        for candidate in candidates:
            if not candidate:
                continue
            current = os.path.realpath(os.path.expanduser(candidate))
    ...
    def _restore_runtime_if_available(self, recovered):
        ...
        try:
            ...
            self.get_logger().warning(
                'Could not restore navigation benchmark runtime state'
            )
    ...
    def record_event(self, event_type, state='', reason='', source='', detail='',
        ...
        self.events.append({
            'wall_time': iso_time(),
            'ros_time': number(self._now_seconds(), 9),
            'session_id': self.session_id,
            'run_id': run_id,
    ...
    def _save_path(self, message, path_type, version):
        wall = iso_time()
        ros_stamp = self._stamp_seconds(message.header.stamp)
        ros_stamp = ros_stamp or self._now_seconds()
        run_id = self.active_task.run_id if self.active_task else ''
    ...
    def _current_pose(self):
        transform = self.tf_buffer.lookup_transform(
            self.map_frame, self.base_frame, Time(), Duration(seconds=0.03)
        )
        ...
        stamp = self._stamp_seconds(transform.header.stamp)
        age_ms = (
            max(0.0, (self._now_seconds() - stamp) * 1000.0)
    ...
    def task_status_callback(self, message):
        ...
        if len(fields) != 4:
            self.get_logger().warning(
                f'Invalid benchmark task status: {message.data!r}'
            )
            return
        ...
        try:
            ...
            self.get_logger().warning(
                f'Invalid benchmark target id: {requested_text!r}'
            )
            return
        with self.lock:
            if state == 'running':
        ...
        with self.lock:
            ...
            try:
                ...
                try:
                    ...
                    if rclpy.ok():
                        self.get_logger().info(
                            'Navigation benchmark final report: '
                            f'{json_path}, {text_path}'
                        )
                except (OSError, ValueError) as error:
                    self.get_logger().error(
                        'Failed to generate navigation benchmark report: '
                        f'{error}'
                    )
    ...
    try:
        ...
        if node is not None:
            node.get_logger().fatal(
                f'Navigation benchmark initialization failed: {error}'
            )
        else:
            print(f'Navigation benchmark initialization failed: {error}')
        raise
    finally:
        if node is not None:
            node.shutdown()
