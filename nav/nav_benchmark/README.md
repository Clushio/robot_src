...
`nav_benchmark` is a read-only ROS2 logger for long-running Ranger navigation
acceptance tests. It does not publish goals or velocity commands and does not
modify planner/controller state.

## Automatic lifecycle

`robot_r/launch/3navlocations.launch.py` includes
`navigation_benchmark.launch.py`. Clicking **AutoNAV** in the GUI therefore starts
...
```bash
ros2 run nav_benchmark analyze_results.py
```

Or combine explicit fixed-route/reroute sessions:

```bash
ros2 run nav_benchmark analyze_results.py SESSION_A SESSION_B \
