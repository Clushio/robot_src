...
public:
    GroundDetector(int ns, double sh, int ni, int nl, double ts, double td);
    ...
    std::vector<pcl::PointCloud<PointType>::Ptr> getRawPointCloudWithLabel();
...
protected:
    ...
    void plane_fitter_(pcl::PointCloud<PointType>::Ptr rpc, pcl::PointCloud<PointType>::Ptr gm, Eigen::Vector4d &plane, float th);
