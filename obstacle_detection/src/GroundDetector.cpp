...
GroundDetector::GroundDetector(int ns, double sh, int ni, int nl, double ts, double td): num_segments_(ns),
sensor_height_(sh), num_iter_(ni), num_lpr_(nl), th_seeds_(ts), th_dist_(td)
{
    ...
    mThresholds.push_back(th_dist_); // 中
    mThresholds.push_back(th_dist_); // 前
    mThresholds.push_back(th_dist_); // 后
...
void GroundDetector::DetectGround(pcl::PointCloud<PointType>::Ptr RawPointCloud)
{
    ...
    for(int i=0; i<num_segments_; i++)
    {
        mthreads[i] = new std::thread(&GroundDetector::DetectGroundBySegment, this, 
                                     mRPCs[i], mSortAxisZMeas[i], std::ref(mPlanes[i]), mGroundMeas[i], std::ref(mThresholds[i]));
    }
    ...
    // printf("ground detection time %f ms \n \n", t_ground.toc());

}

// 填充 mRPCs, mSortAxisZMeas
void GroundDetector::pc_segment_by_long_axis()
{
...
void GroundDetector::DetectGroundBySegment(pcl::PointCloud<PointType>::Ptr rpc, pcl::PointCloud<PointType>::Ptr sazm,
                            Eigen::Vector4d &plane, pcl::PointCloud<PointType>::Ptr gm, float &th)
{
    ...
    plane_fitter_(rpc, gm, plane, th);

}

// 下面的函数改成向量相关
void GroundDetector::mirror_reflection_(pcl::PointCloud<PointType>::Ptr pc)
{
...
// estimate plane 
void GroundDetector::estimate_plane_(pcl::PointCloud<PointType>::Ptr gm, Eigen::Vector4d &plane)
{
    Eigen::Matrix3f cov;
...
void GroundDetector::extract_initial_seeds_(pcl::PointCloud<PointType>::Ptr sazm, pcl::PointCloud<PointType>::Ptr gm)
{
    gm->clear();
...
void GroundDetector::plane_fitter_(pcl::PointCloud<PointType>::Ptr rpc, 
                                   pcl::PointCloud<PointType>::Ptr gm, Eigen::Vector4d &plane, float th)
{
...
pcl::PointCloud<PointType>::Ptr GroundDetector::getMergedRawPointCloud()
{
    return mMergedRawPointCloud;
}

std::vector<pcl::PointCloud<PointType>::Ptr> GroundDetector::getDetect3dGround()
{
    return mGroundMeas;
}

std::vector<pcl::PointCloud<PointType>::Ptr> GroundDetector::getRawPointCloudWithLabel()
{
    return mRPCs;
}