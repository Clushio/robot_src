...
private:
    ...
    pcl::PointCloud<velodyne_pcl::PointXYZIRT>::Ptr laserCloudIn; // 存储原始velodyne数据
...
public:
    ObjectDetector()
    {
        laserCloudIn.reset(new pcl::PointCloud<velodyne_pcl::PointXYZIRT>());
        ...
        Zpassthrough.setFilterLimits(z_lower_boundary_, 10000);

        subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>("/velodyne_points", 1, &ObjectDetector::cloudHandler, this);  
    ...
    void cloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {
        ...
        float min_rel_timestamp = 1e15;
        for(int i=0; i<laserCloudIn->points.size(); i++)
        {
             if(laserCloudIn->points[i].time<min_rel_timestamp)
                 min_rel_timestamp = laserCloudIn->points[i].time;
        }
        ...
        for(int i=0; i<laserCloudIn->points.size(); i++)
        {
            ...
            point.intensity = (laserCloudIn->points[i].time - min_rel_timestamp) / 0.1;
            laserCloudIn2->push_back(point);    
        }
        ...
        std::cout << "unground_pc->size(): " << unground_pc->size() << std::endl;
