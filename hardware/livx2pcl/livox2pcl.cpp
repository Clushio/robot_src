...
class LivoxRepublisher : public rclcpp::Node
{
public:
  LivoxRepublisher()
  : Node("livox_repub")
  {
    ...
    subscription_ = create_subscription<CustomMsg>(
      "/livox/lidar", 100,
      std::bind(
        &LivoxRepublisher::LivoxMsgCallback, this,
        std::placeholders::_1));
    RCLCPP_INFO(get_logger(), "start livox_repub");
  }
  ...
  void LivoxMsgCallback(CustomMsg::ConstSharedPtr livox_msg_in)
  {
    ...
    for (const auto & livox_msg : livox_data_) {
      ...
      for (unsigned int i = 0; i < livox_msg->point_num; ++i) {
        PointType point;
        point.x = livox_msg->points[i].x;
        point.y = livox_msg->points[i].y;
        point.z = livox_msg->points[i].z;
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<LivoxRepublisher>());
  rclcpp::shutdown();
  return 0;
}
